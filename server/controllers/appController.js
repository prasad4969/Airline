...
async function searchFlight(req, res) {
    ...
    try {
        ...
        const flights = await Flight.find(searchCriteria).select('flightNo to from category date time totalSeats price');
...
const insertFlight = async(req, res)=>{
   ...
   await Flight.insertMany(arr);

   return res.json({message : "ALL flights Inserted Successfully!"});
