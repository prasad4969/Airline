...
const FlightManage = () => {
  ...
  const fetchFlights = async () => {
    try {
      const response = await fetch("http://localhost:8000/api/admin/getallflights", {
        headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/admin/deleteflight/${id}`, {
        method: "DELETE",
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:8000/api/admin/updateFlight/${editingFlight._id}`, {
        method: "PUT",
        headers: {
