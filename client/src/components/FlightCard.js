...
function FlightCard({flightData, setBookFlightData }) {
    ...
    let { flightNo, from, to, category, date, departureTime, duration, arrivalTime, price, aircraft, airline, stops } = flightData;
    console.log(date)
    ...
    async function book_flight(event) {
        ...
        try {
            setIsLoading(true)
            const response = await fetch('http://127.0.0.1:8000/api/canbook', {
                method: 'POST',
                headers: {
                ...
                <div className="flight-area">
                    ...
                    <div className="flight-button">
                        <div className="amount">
                            ...
                            <h5 className="price">{price && price.toLocaleString('en-US', {style : 'currency', currency : 'INR'})}</h5>
                        </div>
